...
export default function App() {
  ...
  const Avatar = ({ seed, size = 'md' }) => {
    ...
    const idx = (seed?.charCodeAt?.(0) ?? 0) % 5;
  ...
  const toggleTask = (id) => {
    setTasks(tasks.map(t => t.id === id
      ? { ...t, completed: !t.completed, completedBy: !t.completed ? userProfile.name : null }
      : t
    ));
  };
  ...
  const addSubtask = () => {
    ...
    setTasks(tasks.map(t => t.id === activeTask.id
      ? { ...t, subtasks: [...t.subtasks, { id: Math.max(...t.subtasks.map(s => s.id), 0) + 1, text: newSubtaskText, completed: false }] }
      : t
    ));
    setNewSubtaskText('');
  };
    ...
    setTasks(tasks.map(t => t.id === activeTask.id
      ? {
        ...
        comments: [
          ...(t.comments || []),
          {
            id: Math.max(...((t.comments?.length > 0 ? t.comments.map(c => c.id) : [0])), 0) + 1,
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-black flex flex-col items-center justify-center p-6 relative overflow-hidden">
      ...
      <div className="z-10 text-center w-full max-w-sm">
        ...
        <div className="space-y-4">
          <button
            onClick={handleLogin}
            className="w-full bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 p-4 rounded-xl flex items-center justify-center gap-3 shadow-sm hover:shadow-md transition-all active:scale-95 group"
          >
    ...
    <div className="pb-24 pt-6 px-4 h-screen overflow-y-auto">
      ...
      <div className="bg-white dark:bg-gray-900 rounded-2xl p-6 shadow-sm border border-gray-100 dark:border-gray-800 mb-6 text-center">
        ...
        <input
          ...
          className="text-center text-xl font-bold text-gray-900 dark:text-white bg-transparent border-b border-gray-200 dark:border-gray-700 pb-1 mb-4 focus:outline-none focus:border-blue-500 w-full"
        />
        <div className="bg-gray-50 dark:bg-gray-800 p-3 rounded-xl flex items-center justify-between gap-3">
          <div className="flex items-center gap-2">
            ...
            <div className="text-left">
              ...
              <p className="text-sm font-mono text-gray-700 dark:text-gray-200">{userProfile.id}</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="space-y-2">
          ...
          <div className="p-4 bg-white dark:bg-gray-900 rounded-xl border border-gray-100 dark:border-gray-800">
            <div className="flex items-center justify-between mb-3">
              ...
              <button
                ...
                className={`px-4 py-2 rounded-lg text-sm font-semibold ${pushPermission === 'granted' ? 'bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-300' : 'bg-blue-600 text-white'}`}
              >
                {pushPermission === 'granted' ? 'Aktiviert' : 'Aktivieren'}
              </button>
            </div>
        ...
        <div
          className="flex items-center justify-between p-4 bg-white dark:bg-gray-900 rounded-xl border border-gray-100 dark:border-gray-800 cursor-pointer hover:bg-red-50 dark:hover:bg-red-900/10 group"
    ...
    <div className="pb-24 pt-6 px-4">
      <header className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400">
            Heute sind {tasks.filter(t => t.date === new Date().toISOString().split('T')[0] && !t.completed).length} offene Aufgaben
          </p>
        </div>
        <div className="flex items-center gap-2">
          <button onClick={() => setCurrentView('settings')} className="p-3 bg-gray-100 dark:bg-gray-800 rounded-full hover:bg-gray-200 dark:hover:bg-gray-700">
            <Settings size={24} className="text-gray-600 dark:text-gray-300" />
          </button>
          <button onClick={() => setIsDarkMode(!isDarkMode)} className="p-3 bg-gray-100 dark:bg-gray-800 rounded-full hover:bg-gray-200 dark:hover:bg-gray-700">
            {isDarkMode ? <Sun size={24} className="text-yellow-500" /> : <Moon size={24} className="text-gray-600" />}
          </button>
        </div>
      </header>
      ...
      <div className="flex gap-2 mb-6 border-b border-gray-200 dark:border-gray-800 overflow-x-auto">
        <button
          ...
          className={`px-4 py-3 font-semibold whitespace-nowrap border-b-2 transition-all ${dashboardTab === 'active' ? 'border-blue-600 text-blue-600 dark:text-blue-400' : 'border-transparent text-gray-600 dark:text-gray-400'}`}
        >
          Offen ({tasks.filter(t => !t.completed && t.date <= new Date().toISOString().split('T')[0]).length})
        </button>
        <button
          onClick={() => setDashboardTab('today')}
          className={`px-4 py-3 font-semibold whitespace-nowrap border-b-2 transition-all ${dashboardTab === 'today' ? 'border-blue-600 text-blue-600 dark:text-blue-400' : 'border-transparent text-gray-600 dark:text-gray-400'}`}
        >
          Heute ({tasks.filter(t => t.date === new Date().toISOString().split('T')[0]).length})
        </button>
        <button
          onClick={() => setDashboardTab('completed')}
          className={`px-4 py-3 font-semibold whitespace-nowrap border-b-2 transition-all ${dashboardTab === 'completed' ? 'border-blue-600 text-blue-600 dark:text-blue-400' : 'border-transparent text-gray-600 dark:text-gray-400'}`}
        >
      ...
      <div className="space-y-2 mb-6">
        {(dashboardTab === 'active'
          ? tasks.filter(t => !t.completed && t.date <= new Date().toISOString().split('T')[0])
          : dashboardTab === 'today'
          ...
          return (
            <div
              key={task.id}
              onClick={() => { setActiveTask(task); setCurrentView('taskDetail'); }}
              className={`p-4 rounded-xl border transition-all cursor-pointer ${task.completed ? 'bg-gray-50 dark:bg-gray-900/50 border-gray-200 dark:border-gray-800' : 'bg-white dark:bg-gray-900 border-gray-200 dark:border-gray-800 shadow-sm hover:shadow-md'}`}
            >
              <div className="flex items-start gap-3">
                <button
                  onClick={(e) => { e.stopPropagation(); toggleTask(task.id); }}
                  className={`mt-1 flex-shrink-0 w-5 h-5 rounded-full border-2 flex items-center justify-center transition-all ${task.completed ? 'bg-green-500 border-green-500' : 'border-gray-300 dark:border-gray-600 hover:border-green-500'}`}
                >
                  {task.completed && <Check size={16} className="text-white" />}
                </button>

                <div className="flex-1 min-w-0">
                  <p className={`font-semibold ${task.completed ? 'text-gray-500 dark:text-gray-600 line-through' : 'text-gray-900 dark:text-white'}`}>
                    {task.title}
                  </p>
                  ...
                  <div className="flex items-center gap-2 mt-1 flex-wrap">
                    ...
                    {priority && (
                      <span className={`inline-flex items-center gap-1 text-xs px-2 py-1 rounded-full ${priority.color}`}>
                  ...
                  <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
                    {task.date} {task.time ? `um ${task.time}` : ''}
                  </p>
                </div>
        ...
        {(dashboardTab === 'active'
          ...
          <p className="text-center text-gray-400 py-6">Keine Aufgaben</p>
        )}
      </div>

      <button
        onClick={() => setCurrentView('addTask')}
        className="w-full py-4 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-xl shadow-lg active:scale-95 transition-all flex items-center justify-center gap-2"
      >
    ...
    return (
      <div className="pb-24 pt-6 px-4 h-screen overflow-y-auto">
        <header className="mb-6 flex items-center justify-between">
          ...
          {/* ✅ FIX: block body statt Semikolon-Chain */}
          <button
            onClick={() => {
          ...
          <div className="bg-white dark:bg-gray-900 p-4 rounded-xl border border-gray-100 dark:border-gray-800">
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                className={`w-6 h-6 rounded-full border-2 flex items-center justify-center ${activeTask.completed ? 'bg-green-500 border-green-500' : 'border-gray-300 dark:border-gray-600'}`}
              >
                {activeTask.completed && <Check size={16} className="text-white" />}
              </button>

              <span className="text-gray-900 dark:text-white">{activeTask.completed ? 'Erledigt' : 'Offen'}</span>
            </div>
          </div>
        </div>
        ...
        <div className="mb-20">
          ...
          <div className="space-y-2 mb-4">
            {activeTask.subtasks?.map(s => (
              <div key={s.id} className="flex items-center gap-3 p-3 bg-gray-50 dark:bg-gray-800 rounded-xl">
                <button
                  onClick={() => toggleSubtask(activeTask.id, s.id)}
                  className={`flex-shrink-0 w-5 h-5 rounded border-2 flex items-center justify-center ${s.completed ? 'bg-green-500 border-green-500' : 'border-gray-300 dark:border-gray-600'}`}
                >
          ...
          <div className="flex gap-2">
            <input
              ...
              className="flex-1 p-3 bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl text-sm dark:text-white focus:outline-none focus:ring-1 focus:ring-blue-500"
              onKeyDown={(e) => e.key === 'Enter' && addSubtask()}
            />
            <button onClick={addSubtask} className="bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-300 p-3 rounded-xl hover:bg-gray-200 dark:hover:bg-gray-700">
              <Plus size={20} />
            </button>
          </div>
        </div>
        ...
        <div className="mb-20">
            ...
            {activeTask.comments && activeTask.comments.length > 0 ? (
              activeTask.comments.map(c => (
                <div key={c.id} className="flex gap-3">
                  <Avatar seed={c.author} size="sm" />
                  <div className="bg-gray-100 dark:bg-gray-800 p-3 rounded-r-xl rounded-bl-xl max-w-[80%]">
                    <p className="text-xs text-gray-500 mb-1">{c.author}</p>
              ...
              <p className="text-center text-gray-400 text-sm py-4">Noch keine Nachrichten.</p>
            )}
          </div>
          ...
          <div className="flex gap-2 items-center fixed bottom-6 left-4 right-4 bg-white dark:bg-black p-2 border border-gray-200 dark:border-gray-800 rounded-full shadow-lg">
            <input
              ...
              className="flex-1 bg-transparent px-4 py-2 focus:outline-none text-gray-900 dark:text-white"
            />
            <button onClick={sendComment} className="p-2 bg-blue-600 rounded-full text-white hover:bg-blue-700">
              <Send size={18} />
            </button>
          </div>
        </div>
    ...
    <div className="h-full bg-white dark:bg-black z-30 absolute top-0 left-0 w-full flex flex-col">
      ...
      <div className="flex-1 overflow-y-auto px-4 pb-24">
        <div className="space-y-6 pt-4">
          <div>
            <label className="block text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-2">Titel</label>
            <input
              type="text"
              value={newTaskTitle}
              onChange={(e) => setNewTaskTitle(e.target.value)}
              placeholder="z.B. Milch, Meeting..."
              className="w-full p-4 bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl focus:ring-2 focus:ring-blue-500 focus:outline-none text-lg dark:text-white"
              autoFocus
            />
          </div>
            ...
            <div className="grid grid-cols-4 gap-2">
              {CATEGORIES.map(cat => (
                <button
                  ...
                  className={`flex flex-col items-center justify-center p-2 rounded-xl border transition-all ${newTaskCategory === cat.id ? 'border-blue-500 ring-1 bg-blue-50 dark:bg-blue-900/20' : 'border-gray-100 dark:border-gray-800 dark:bg-gray-900'}`}
                >
          ...
          <div>
            ...
            <div className="flex gap-2">
              {PRIORITIES.map(prio => (
                <button
                  key={prio.id}
                  onClick={() => setNewTaskPriority(prio.id)}
                  className={`flex-1 py-3 rounded-xl border flex flex-col items-center justify-center gap-1 ${newTaskPriority === prio.id ? prio.activeClass : 'border-gray-200 dark:border-gray-800 dark:text-gray-400'}`}
                >
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-2">Datum</label>
              <input
                type="date"
                value={newTaskDate}
                onChange={(e) => setNewTaskDate(e.target.value)}
                className="w-full p-3 bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl dark:text-white dark:[color-scheme:dark]"
              />
            </div>
            <div>
              <label className="block text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-2">Uhrzeit</label>
              <input
                type="time"
                value={newTaskTime}
          ...
          <div>
            ...
            <div className="flex gap-2 overflow-x-auto pb-2 no-scrollbar">
              <button
                onClick={() => { setAssignType('private'); setAssignTargetId(null); }}
                className={`px-4 py-2 rounded-full border whitespace-nowrap text-sm ${assignType === 'private' ? 'bg-gray-800 text-white dark:bg-gray-700' : 'border-gray-200 dark:border-gray-700 dark:text-gray-300'}`}
              >
                Nur ich
              </button>
              ...
              {groups.map(g => (
                <button
                  key={g.id}
                  onClick={() => { setAssignType('group'); setAssignTargetId(g.id); }}
                  className={`px-4 py-2 rounded-full border whitespace-nowrap text-sm ${assignType === 'group' && assignTargetId === g.id ? 'bg-indigo-600 text-white' : 'border-gray-200 dark:border-gray-700 dark:text-gray-300'}`}
                >
                  {g.name}
                </button>
              ))}
              ...
              {friends.map(f => (
                <button
                  key={f.id}
                  onClick={() => { setAssignType('friend'); setAssignTargetId(f.id); }}
                  className={`px-4 py-2 rounded-full border whitespace-nowrap text-sm ${assignType === 'friend' && assignTargetId === f.id ? 'bg-purple-600 text-white' : 'border-gray-200 dark:border-gray-700 dark:text-gray-300'}`}
                >
                  {f.name}
                </button>
              ))}
          ...
          <button
            ...
            className={`w-full py-4 rounded-xl text-white font-bold text-lg shadow-lg ${!newTaskTitle || !newTaskDate ? 'bg-gray-300 dark:bg-gray-800' : 'bg-blue-600 hover:bg-blue-700'}`}
          >
            Speichern
          </button>
        </div>
    ...
    <div className="pb-24 pt-6 px-4 h-screen overflow-y-auto">
      ...
      <div className="flex gap-2 mb-6">
        <input
          ...
          className="flex-1 p-3 bg-gray-50 dark:bg-gray-900 border rounded-lg dark:border-gray-800 dark:text-white focus:outline-none focus:ring-1 focus:ring-indigo-500"
        />
        <button onClick={createGroup} className="bg-indigo-600 text-white px-4 rounded-lg"><Plus /></button>
      </div>

      {groups.map(g => (
        <div
          key={g.id}
          onClick={() => { setActiveGroup(g); setCurrentView('dashboard'); }}
          className="p-5 bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800 rounded-2xl mb-3 cursor-pointer shadow-sm hover:border-indigo-200 transition-all"
        >
          <div className="flex justify-between items-center mb-3">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-gray-500 dark:text-gray-400">{getGroupProgress(g.id)}% Erledigt</p>
              </div>
            </div>
          ...
          <div className="w-full bg-gray-100 dark:bg-gray-800 h-2 rounded-full overflow-hidden">
            <div className="bg-indigo-500 h-full rounded-full transition-all duration-500" style={{ width: `${getGroupProgress(g.id)}%` }}></div>
          </div>
        </div>
      ))}
    </div>
    ...
    <div className="pb-24 pt-6 px-4 h-screen overflow-y-auto">
      ...
      <div className="flex gap-2 mb-4">
        <input
          ...
          placeholder="Name..."
          className="flex-1 p-3 bg-gray-50 dark:bg-gray-900 border rounded-lg dark:border-gray-800 dark:text-white focus:outline-none focus:ring-1 focus:ring-purple-500"
        />
        <button onClick={addFriend} className="bg-purple-600 text-white px-4 rounded-lg"><Plus /></button>
      </div>

      {friends.map(f => (
        <div key={f.id} className="p-4 bg-white dark:bg-gray-900 border dark:border-gray-800 rounded-xl mb-2 flex items-center gap-3">
          <Avatar seed={f.name} />
