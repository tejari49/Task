...
export default function App() {
  ...
  const handleLogin = async () => {
    ...
    if (!auth) {
      setAuthError('Firebase Auth nicht initialisiert');
      return;
    }
  ...
  const addFriend = () => {
    ...
    setFriends([...friends, { id: Math.max(...friends.map(f => f.id), 0) + 1, name: newFriendName, avatar: newFriendName }]);
    setNewFriendName('');
  };

  const createGroup = () => {
    if (!newGroupName) return;
    setGroups([...groups, { id: Math.max(...groups.map(g => g.id), 0) + 1, name: newGroupName, memberCount: 1 }]);
    setNewGroupName('');
  };
    ...
    return (
      <div className="pb-24 pt-6 px-4 h-screen overflow-y-auto">
        <header className="mb-6 flex items-center justify-between">
          ...
          <button onClick={handleDeleteTask} className="p-2 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-full text-red-600 dark:text-red-400"><Trash2 size={24} /></button>
        </header>
        <div className="space-y-4 mb-6">
          ...
          <div className="bg-white dark:bg-gray-900 p-4 rounded-xl border border-gray-100 dark:border-gray-800">
            ...
            <div className="flex items-center gap-2">
              <button onClick={handleToggleStatus} className={`w-6 h-6 rounded-full border-2 flex items-center justify-center ${activeTask.completed ? 'bg-green-500 border-green-500' : 'border-gray-300 dark:border-gray-600'}`}>{activeTask.completed && <Check size={16} className="text-white" />}</button>
